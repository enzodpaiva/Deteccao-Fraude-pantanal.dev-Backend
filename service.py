...
fraud_detection_runner = bentoml.xgboost.get("fraud_detection_xgb:latest").to_runner()
...
def random_transaction(input) -> dict:
    ...
    random_transaction = cursor.execute('''SELECT * FROM transactions WHERE Class=1 ORDER BY RANDOM() LIMIT 1''')
    random_transaction = [float(value) for value in random_transaction.fetchall()[0]]
    filtered_random_transaction = [random_transaction[-2], random_transaction[1], random_transaction[2], random_transaction[3], random_transaction[4], random_transaction[5], random_transaction[6], random_transaction[7], random_transaction[8], random_transaction[9], random_transaction[10], random_transaction[11], random_transaction[12], random_transaction[14], random_transaction[16], random_transaction[17], random_transaction[18], random_transaction[19], random_transaction[20], random_transaction[21]]
    filtered_random_transaction[0] = np.log10(filtered_random_transaction[0] + 0.00001)

    transaction_class = fraud_detection_runner.predict.run([filtered_random_transaction])
